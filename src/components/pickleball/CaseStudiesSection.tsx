...
export const CaseStudiesSection = () => {
  ...
  return (
    <section
      id="case-studies"
      className={`bg-background text-white font-sans border-t border-[#1a3a2d] ${sectionSpacing}`}
    >
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="flex flex-col items-center justify-center text-center mb-16">
          <motion.div
            {...fadeUp}
            className={`${sectionEyebrow} mb-4 font-dela text-[#a8c78c]`}
          >
          ...
          <motion.p
            {...fadeUp}
            className={`${sectionBody} max-w-2xl font-light text-white/60`}
          >
            From 3-day professional leagues to corporate events and national
            tournaments.
          </motion.p>
        </div>
        ...
        <div className="mb-8">
          <h3 className="font-dela text-xl sm:text-2xl md:text-[1.8rem] text-white mb-2">
            Case Studies
          </h3>
          <p className={`${sectionBody} mb-6 font-light text-white/60`}>
            Detailed breakdowns of our flagship events and complete tournament
            operations.
          </p>
        ...
        <motion.div
          {...fadeUp}
          className="group relative bg-card border border-white/5 rounded-2xl overflow-hidden flex flex-col md:flex-row mb-12 hover:border-white/10 transition-colors cursor-pointer"
        >
          {/* Left: Image */}
          <div className="w-full md:w-1/2 relative h-[300px] md:h-auto overflow-hidden">
            <div className="absolute top-4 left-4 z-20">
              <span className="bg-lime text-[#102319] text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded">
                Featured
              </span>
            </div>
            <img
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-3 mt-auto">
                {featuredStudy.stats.map((st, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <span className="text-[#d0e999] font-dela text-lg">
                      {st.val}
                    </span>
                ...
                <div className="p-6 flex flex-col flex-1">
                  <div className="flex items-center gap-3 text-[10px] text-white/50 uppercase tracking-wider font-semibold mb-3">
                    <span className="flex items-center gap-1">
                      <MapPin className="w-3 h-3 text-[#d0e999]" />{" "}
                      {cs.location}
                    </span>
                    <span className="w-1 h-1 bg-white/20 rounded-full"></span>
        ...
        <div>
          <div className="">
            <h3 className="font-dela text-xl sm:text-2xl md:text-[1.8rem] text-white">
              4 Pillars of Production
            </h3>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 items-start gap-2">

...
export const GallerySection = () => {
  ...
  const galleryItems = [
    { src: pickleballEventBg, caption: "WPPL Season 1 — Official Stage & LED Setup" },
    { src: "https://images.unsplash.com/photo-1595435934249-5df7ed86e1c0?q=80&w=1200&auto=format&fit=crop", caption: "WPPL Season 1 — Match action on court" },
    { src: pickleballHeroBg, caption: "WPPL Season 1 — Trophy Ceremony" },
    { src: "https://images.unsplash.com/photo-1554068865-c72eb88d3e87?q=80&w=1200&auto=format&fit=crop", caption: "Chennai Super Champs — Court branding" },
    { src: "https://images.unsplash.com/photo-1622279457486-640c43431653?q=80&w=1200&auto=format&fit=crop", caption: "Bangalore Jawans — Event branding" },
    { src: "https://images.unsplash.com/photo-1612872087720-bb876e2e67d1?q=80&w=1200&auto=format&fit=crop", caption: "TNPA Women's Day — Players action shot" },
  ];
  ...
  const getStyle = (index: number) => {
    ...
    if (abstractDiff === 0) {
      ...
      return { x: abstractDiff > 0 ? "100%" : "-100%", scale: 0.8, zIndex: 0, opacity: 0, rotateY: 0 };
    }
  };
  ...
  return (
    <section className="py-24 md:py-32 px-4 relative overflow-hidden" style={{ backgroundColor: '#253e35' }}>
      <div className="max-w-[1400px] mx-auto">
        ...
        <motion.h2 {...fadeUp} className="font-dela text-4xl sm:text-5xl md:text-6xl text-white text-center mb-6 leading-tight">
          See What We Build.
        </motion.h2>
        ...
        <motion.p {...fadeUp} className="text-center text-white/50 text-sm md:text-base max-w-2xl mx-auto mb-20 font-light leading-relaxed">
          From the LED wall to the Instagram reel — this is what EyeLevel looks like in the field.
        </motion.p>
        ...
        <div className="relative w-full h-[400px] md:h-[450px] lg:h-[550px] flex items-center justify-center perspective-[1200px]">
            ...
            {galleryItems.map((item, index) => {
              ...
              return (
                <motion.div
                  ...
                  className={`absolute w-full max-w-[85vw] md:max-w-4xl h-full aspect-[4/3] sm:aspect-video rounded-[2rem] overflow-hidden shadow-2xl ${currentStyle.x === "0%" ? 'cursor-default' : 'cursor-pointer'
                    }`}
                >
                  <img
                    src={item.src}
                  ...
                  <div className={`absolute inset-0 bg-[#253e35] transition-opacity duration-700 pointer-events-none ${currentStyle.x === "0%" ? 'opacity-0' : 'opacity-40'
                    }`}></div>
          ...
          <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-between px-4 md:px-12 z-20 pointer-events-none">
            <button
              onClick={handlePrev}
              className="w-12 h-12 rounded-full bg-[#253e35]/50 backdrop-blur-md border border-white/20 text-white flex items-center justify-center pointer-events-auto hover:bg-[#d0e999] hover:text-[#253e35] hover:border-[#d0e999] transition-all duration-300"
            >
        ...
        <div className="flex items-center justify-center gap-3 mt-12 relative z-20">
          {galleryItems.map((_, i) => (
            <button
              ...
              className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${currentIndex === i
                ? "bg-[#d0e999] scale-125"

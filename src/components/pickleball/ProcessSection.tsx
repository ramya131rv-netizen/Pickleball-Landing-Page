...
import image1 from "@/assets/howWeDo/image1.webp";
import image2 from "@/assets/howWeDo/image2.webp";
import image3 from "@/assets/howWeDo/image3.webp";
...
export const ProcessSection = () => {
  ...
  const stepImages = [
    image1, // 01: Strategize & Plan (Team huddle with strategy boards)
    image2, // 02: Produce & Create (Live Pickleball Event Action)
    image3, // 03: Deliver & Report (Analytics / ROI Report)
  ];

  return (
    <section
      id="process"
      className={`w-full bg-background-deep font-sans relative overflow-hidden ${sectionSpacing}`}
    >
      {/* Subtle Glow inside Section */}
      <div className="absolute top-0 left-0 w-[500px] h-[500px] /5 blur-[120px] rounded-full -translate-x-1/2 -translate-y-1/2 pointer-events-none z-0"></div>
      ...
      >
        ...
        <div className="mb-16 relative z-10">
          <motion.div
            {...fadeUp}
            className={`${sectionEyebrow} text-[#a8c78c] mb-4`}
          >
            How It Works
          </motion.div>
          ...
          <motion.p
            {...fadeUp}
            className={`${sectionBody} text-white/60 font-light`}
          >
          ...
          <div className="w-full lg:w-1/2 relative flex flex-col">
                ...
                <motion.div
                  ...
                  className={`group relative flex gap-6 sm:gap-8 items-start cursor-pointer transition-all duration-500 ${
                    activeStep === i
                      ...
                      : "opacity-40 scale-[0.98] hover:opacity-70"
                  }`}
                >
                  {/* Number Badge */}
                  <div
                    className={`w-14 h-12 shrink-0 rounded-[14px] flex items-center justify-center font-dela text-xl relative z-10 transition-all duration-500 ${
                      activeStep === i
                        ? "border border-[#dce288]/50 /20 text-[#dce288] shadow-[0_0_20px_rgba(220,226,136,0.2)]"
                  ...
                  <div className="pt-1">
                    <h3
                      className={`font-dela text-2xl tracking-wide mb-3 transition-colors duration-500 ${
                        activeStep === i ? "text-white" : "text-white/70"
                      }`}
                    >
                      {step.title}
                    </h3>
                    <p className="text-white/60 text-sm leading-relaxed font-light mb-6 line-clamp-3 w-full lg:pr-10">
                      {step.desc}
                    </p>
                        ...
                        >
                          <div
                            className={`w-3 h-3 rounded-full border-[1.5px] transition-colors duration-300 ${
                              activeStep === i
                                ? "border-[#dce288]/60 group-hover/item:border-[#dce288]"
                                : "border-white/20"
                            }`}
                          ></div>
                          <span
                            className={`text-[11px] sm:text-xs font-light transition-colors duration-300 ${
                              activeStep === i
            ...
            >
              <button
                ...
                className="px-8 py-4 bg-lime text-[#1a2e22] text-sm font-bold rounded-[14px] transition-all duration-300 hover:-translate-y-1 flex items-center gap-2"
              >
